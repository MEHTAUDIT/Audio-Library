...
public class FlywayConfig {
    ...
    private void createTenantTables(String schemaName) {
        try (Connection connection = dataSource.getConnection();
            ...
            statement.execute(
                    "CREATE TABLE IF NOT EXISTS \"" + schemaName + "\".series (" +
                            "    id UUID PRIMARY KEY," +
            ...
            statement.execute(
                    "CREATE TABLE IF NOT EXISTS \"" + schemaName + "\".audio_files (" +
                            ...
                            "    media_type VARCHAR(10) DEFAULT 'AUDIO'," +  // ADDED: video support
                            "    series_id UUID," +                            // ADDED: series grouping
                            "    series_order INT DEFAULT 0," +                // ADDED: order within series
                            "    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP," +
                            "    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP" +
                            ")"
            );
